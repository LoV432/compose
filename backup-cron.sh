...
if [[ $(date +%u) == "6" ]] || [[ $(date +%u) == "3" ]]; then
  ...
  ssh -oStrictHostKeyChecking=no -oBatchMode=yes root@192.168.1.102 'umask go=; sysupgrade -b /tmp/router-backup.tar.gz'
  ...
  scp -O -oStrictHostKeyChecking=no -oBatchMode=yes root@192.168.1.102:/tmp/router-backup.tar.gz $BACKUP_DIR/openwrt/router-dumbap2-backup.tar.gz
  ...
  ssh -oStrictHostKeyChecking=no -oBatchMode=yes root@192.168.1.102 'rm /tmp/router-backup.tar.gz'
